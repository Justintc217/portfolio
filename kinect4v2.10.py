...
class Game():
    ...
    def testplace(self,mv,obj,board):   ######## ISSUE IS WITH BOARD CHANGING!!
        if board == "failure":
            return "failure"
    ...
    def evaluate(self, board):   # evaluates the board and checks for winning scenerios, awards points to team with a win. 
        ...
        final = [comp_win, player_win, comp_pt, player_pt, dbcheck]
        return final
        ...
        for i1 in range(7):
            ...
            for i2 in range(7):
                ...
                for i3 in range(7):
                    ...
                    dbadd = self.dbchecker(third_board,third_points[4],second_points[4])
                    third_counter = round(third_counter + third_points[0] + third_points[2] + dbadd[0] ,2)
                    for i4 in range(7): 
                        fourth_board = self.testplace(i4,self.objects[1],third_board)
                        fourth_points = self.evaluate(fourth_board)
                        dbadd = self.dbchecker(fourth_board,fourth_points[4],third_points[4])
                        fourth_counter = round(fourth_counter + fourth_points[1] + fourth_points[3] + dbadd[1],2)
                        for i5 in range(7):
                            fifth_board = self.testplace(i5,self.objects[0],fourth_board)
                            fifth_points = self.evaluate(fifth_board)
                            if fifth_points[4]>fourth_points[4]:
                                yeah += 1
                                
                            else:
                                nah += 1
                                
                            dbadd = self.dbchecker(fifth_board,fifth_points[4],fourth_points[4])
                            fifth_counter = round(fifth_counter + fifth_points[0] + fifth_points[2] + dbadd[0],2)
    ...
    def dbchecker(self,board,dbcheck,prevdbcheck = 1):
        dbpoints = .8
        #return [0,0]
        if dbcheck > prevdbcheck:
            for y in range(5):
                for x in range(7):
                    if board[y][x] == self.space and board[y + 1][x] == self.space:
                        dumboard = [list(board[t]) for t in range(6)]
                        #comp db
                        dumboard[y][x] = self.objects[0]
                        dumboard[y + 1][x] = self.objects[0]
                        dbres = self.evaluate(dumboard)
                        if dbres[0] > 1:
                            return [dbpoints,0]
                        #player db
                        dumboard[y][x] = self.objects[1]
                        dumboard[y + 1][x] = self.objects[1]
                        dbres = self.evaluate(dumboard)
                        if dbres[1] > 1:
                            return [0,dbpoints]
            return [0,0]
